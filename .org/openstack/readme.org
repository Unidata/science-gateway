...
*** Define cluster with cluster.tfvar
    :PROPERTIES:
...
Then, modify =~/jetstream_kubespray/inventory/zonca/cluster.tfvar= to specify the number of nodes in the cluster and the size ([[#h-958EA909][flavor]]) of the VMs. For example,
...
Also, note that ~cluster.tfvar~ assumes you are building a cluster at the TACC data center with the sections pertaining to IU commented out. If you would like to set up a cluster at IU, make the necessary modifications located at the end of ~cluster.tfvar~.
...
This script essentially wraps Terraform install scripts to launch the VMs according to ~cluster.tfvar~.
...
You can augment the computational capacity of your cluster by adding nodes. In theory, this is just a simple matter of [[#h-F44D1317][adding worker nodes]] in =jetstream_kubespray/inventory/$CLUSTER/cluster.tfvar= followed by running:
