...
#+OPTIONS: auto-id:t
#+TITLE: Creating JupyterHub VM on Jetstream
#+DATE:  <2017-06-26 Mon>
...
Copy the =jupyterhub_config.py= file to the =~/config/= directory. [[#h:524FAF4B][Subsequently]], you will have to make minor edits to supply the user and admin white list.
...
[[https://github.com/Unidata/xsede-jetstream/blob/master/openstack/readme.md#h:D6B1D4C2][Open ports]] ~80~, ~443~, and ~8000~ on the JupyterHub VM via OpenStack.
...
See [[https://github.com/Unidata/xsede-jetstream/blob/backup/openstack/wrangler.md][Wrangler documentation]] for creating an NFS mount from Wrangler to Jetstream with a mount point of =/wrangler=. Once the mount is established, we are employing [[https://github.com/laurent22/rsync-time-backup][rsync-time-backup]] (which should be installed in =/usr/local/bin= install with ~sudo~ privileges) to do "time machine" style backups to =/wrangler/backup-notebooks=:

#+BEGIN_SRC shell :exports none :tangle ../../../vms/jupyter/setup-backup.sh :shebang "#!/bin/bash"
  if [ "$EUID" -ne 0 ]
...
#+BEGIN_SRC shell :tangle ../../../vms/jupyter/setup-backup.sh
