...
The ~--ignore-daemonsets~ flag is necessary if any pods on that node were provisioned as part of a [[https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/][DaemonSet]], a set of pods that are scheduled to be ran on every node of a cluster.
