...
See [[https://github.com/Unidata/xsede-jetstream/blob/backup/openstack/wrangler.md][Wrangler documentation]] for creating an NFS mount from Wrangler to Jetstream with a mount point of =/wrangler=. Once the mount is established, we are employing [[https://github.com/laurent22/rsync-time-backup][rsync-time-backup]] to do "time machine" style backups to =/wrangler/backup-notebooks=:

#+BEGIN_SRC shell :tangle ../../../vms/jupyter/setup-backup.sh :shebang "#!/bin/bash"
