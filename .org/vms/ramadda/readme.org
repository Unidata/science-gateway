...
*** Enable SSL as RAMADDA Administrator
    :PROPERTIES:
    :CUSTOM_ID: h-54F74519
    :END:

[[#h-224A9684][Once RAMADDA is running]], you'll want to configure RAMADDA for SSL via the administrative account. There is documentation about this topic [[http://ramadda.org/repository/userguide/installing.html#ssl][here]]. The main thing beyond what has been discussed already in this subsection is the Admin → Settings → Site and Contact Information, ensure "Force all connections to be secure" is checked. The ~repository.properties~ file that is referenced in the ~docker-compose.yml~ should be configured properly for SSL.
