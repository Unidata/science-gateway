...
** Port 80
   ...
   :END:
Open port ~80~ on your VM, however you do that so that RAMADDA can serve content via the web port. Port ~80~ requests will be forwarded to ~8080~ inside the RAMADDA Docker container. [[https://github.com/Unidata/xsede-jetstream/tree/secgroups/openstack#h:D6B1D4C2][See here]] for more information on opening ports.
